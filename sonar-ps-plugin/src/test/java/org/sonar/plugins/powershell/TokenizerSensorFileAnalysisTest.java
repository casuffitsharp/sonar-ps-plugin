...
public class TokenizerSensorFileAnalysisTest {
  ...
  public void testExecute() throws IOException {
    SensorContextTester ctxTester =
        SensorContextTester.create(folder.getRoot().getAbsoluteFile().toPath());
    ...
    DefaultInputFile ti =
        new TestInputFileBuilder(folder.getRoot().getAbsolutePath(), key)
            .initMetadata(new String(Files.readAllBytes(baseFile.toPath())))
            .setLanguage(PowershellLanguage.KEY)
            .build();
  ...
  public void testIfFileIsSkipped() throws IOException {
    final String testKey = ".scannerwork.ps1";
    SensorContextTester ctxTester =
        SensorContextTester.create(folder.getRoot().getAbsoluteFile().toPath());
    if (SystemUtils.IS_OS_WINDOWS) {
      ctxTester.settings().setProperty(Constants.PS_EXECUTABLE, "powershell.exe");
    ...
    File baseFile = folder.newFile(testKey);

    FileUtils.copyURLToFile(getClass().getResource("/testFiles/test.ps1"), baseFile);
    DefaultInputFile ti =
        new TestInputFileBuilder(folder.getRoot().getAbsolutePath(), testKey)
            .initMetadata(new String(Files.readAllBytes(baseFile.toPath())))
            .setLanguage(PowershellLanguage.KEY)
            .build();
  ...
  @Test
  public void shouldSkipWhenTokenizerDisabled() throws IOException {
    SensorContextTester ctxTester =
        SensorContextTester.create(folder.getRoot().getAbsoluteFile().toPath());
    ctxTester.settings().setProperty(Constants.SKIP_TOKENIZER, "true");
