...
public class PowershellPlugin implements Plugin {
  ...
  public void define(final Context context) {
    context.addExtension(
        PropertyDefinition.builder(Constants.SKIP_TOKENIZER)
            .name("Skip tokenizer")
            .description("Flag whether to skip tokenizer")
            .defaultValue("false")
            .type(PropertyType.BOOLEAN)
            .build());
    context.addExtension(
        PropertyDefinition.builder(Constants.SKIP_PLUGIN)
            .name("Skip plugin")
            .description("Flag whether to skip plugin")
            .defaultValue("false")
            .type(PropertyType.BOOLEAN)
            .build());
    context.addExtension(
        PropertyDefinition.builder(Constants.PS_EXECUTABLE)
            ...
            .build());
    context.addExtension(
        PropertyDefinition.builder(Constants.TIMEOUT_TOKENIZER)
            .name("Tokenizer timeout")
            .description("Max time in seconds to wait until tokenizer finishes")
            .defaultValue("3600")
            .type(PropertyType.INTEGER)
            .build());
    context.addExtension(
        PropertyDefinition.builder(Constants.FILE_SUFFIXES)
    ...
    context.addExtension(
        PropertyDefinition.builder(Constants.EXTERNAL_RULES_SKIP_LIST)
            .name("External rules reporting skip list")
            .description("List of repoId:ruleId pairs to skip reporting")
            .multiValues(true)
            .type(PropertyType.STRING)
            .build());
